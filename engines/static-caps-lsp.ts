...
function classify(item: any): string | null {
	...
	const b = path.basename(file); const
		n = item.name;
...
function isUserCode(uri = "") {
	const f = uri.replace("file://", "");

	return f.startsWith(PROJECT) && !f.includes("/node_modules/") && !f.endsWith(".d.ts");
}
...
function request(method: string, params: any) {
	const id = nextId++; const pr = new Promise<any>((res, rej) => pending.set(id, { "res": res, "rej": rej }));
...
function notify(method: string, params: any) { frame({ "jsonrpc": "2.0", "method": method, "params": params }); }
...
p.stdout.on("data", (d) => {
	...
	while (true) {
		...
		if (msg.id !== undefined && msg.method) { frame({ "jsonrpc": "2.0", "id": msg.id, "result": null }); } else if (msg.id !== undefined) {
			const pr = pending.get(msg.id);

			pending.delete(msg.id); if (pr) { (msg.error ? pr.rej(msg.error) : pr.res(msg.result)); }
		}
	}
});
function flatten(s: any[], out: any[] = []): any[] {
	for (const x of s ?? []) { out.push(x); if (x.children) { flatten(x.children, out); } }

	return out;
}

async function reach(item: any, visited: Set<string>, trail: string[], found: { "cap": string; "path": string[] }[]) {
	const key = item.uri + JSON.stringify(item.range);

	if (visited.has(key)) { return; } visited.add(key);
	for (const c of (await request("callHierarchy/outgoingCalls", { "item": item }) ?? [])) {
		const cap = classify(c.to);

		if (cap) { found.push({ "cap": cap, "path": [...trail, c.to.name] }); } else if (isUserCode(c.to.uri)) { await reach(c.to, visited, [...trail, c.to.name], found); }
	}
}
...
(async () => {
	...
	try {
		...
		for (const sym of roots) {
			...
			perRoot[sym.name] = { "caps": caps, "paths": found.map((f) => [sym.name, ...f.path]) };
