...
import { builtinCaps, capsOf } from "./analysis/package-capabilities.js";
...
function ensureSiloDir() { if (!existsSync(SILO_DIR)) { mkdirSync(SILO_DIR, { "recursive": true }); writeFileSync(path.join(SILO_DIR, ".gitignore"), "registry.json\nruns.jsonl\n"); } }
...
function saveReg(r: Reg) { ensureSiloDir(); writeFileSync(REGISTRY, JSON.stringify(r, null, 2) + "\n"); }
...
function run(scriptArg: string, args: string[]) {
	const file = path.resolve(scriptArg); const rel = path.relative(PROJECT, file); const src = readFileSync(file, "utf8"); const
		h = sha(src);
	...
	saveReg(reg);
	appendFileSync(LEDGER, JSON.stringify({ "script": rel, "ts": new Date().toISOString(), "sha": h, "exit": exit, "mode": mode } satisfies RunRec) + "\n");
...
interface Baseline { "consumer": Record<string, WsConsumer>; "packages": Record<string, PkgEntry> }
const loadBaseline = (): Baseline => ({ "consumer": {}, "packages": {}, ...(existsSync(DEPS) ? JSON.parse(readFileSync(DEPS, "utf8")) : {}) });

function saveBaseline(b: Baseline) { ensureSiloDir(); writeFileSync(DEPS, JSON.stringify(b, null, 2) + "\n"); }
...
async function auditPackages(b: Baseline): Promise<number> {
	...
	try { pj = JSON.parse(readFileSync(path.join(PROJECT, "package.json"), "utf8")); } catch {
...
async function baseline(args: string[], consumerOnly = false) {
	...
	if (fresh) {
		saveBaseline(b); console.log(`\n  ✓ baseline written to .silo/baseline.json — commit it; re-run gates drift.`);
...
else if (cmd === "ls" || cmd === "audit" || cmd === "baseline") { await runCli(app, { "argv": argv, "exit": false }); } else { run(cmd, argv.slice(1)); }                                           // `silo <script> [args…]` → the runner
