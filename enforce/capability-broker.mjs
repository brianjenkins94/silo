/**
 * PROTOTYPE — enforcing capability broker (injected into the bundle by bundle.ts, runs first).
 *
...
function grant(scope) { granted.add(scope); if (GRANT_LOG) { try { realFs.appendFileSync(GRANT_LOG, scope + "\n"); } catch {} } process.stderr.write(`[broker] granted ${scope}\n`); }
...
function askSync(q) {
	...
	try {
		const n = realFs.readSync(0, buf, 0, 64, null);
...
function netOk(host) {
	...
	return granted.has("net:" + host) || ALLOW.net.some((a) => n === a || n.endsWith("." + a));
}
...
function applyVerdict(v, scope) {
	if (v.behavior === "allow") {
		process.stderr.write(`[broker] JUDICIAL allow ${v.scope || scope}\n`);
...
function bernard(scope) {
	...
	if (askSync("") === token.toLowerCase()) {
		brokenGlass.add(scope); process.stderr.write(`[broker] BERNARD authorized (one-time) ${scope}\n`);
...
export async function gateNet(host) {
	...
	const v = judicial({ "kind": "net", "scope": scope, "host": host, ...ctx() });
...
export function gateFsSync(op, p) {
	const abs = String(p); const
		scope = `fs:${op}:${abs}`;

	if (redline(scope)) { return bernard(scope); }
	if (fsOk(abs)) { return; }
	const v = judicial({ "kind": "fs", "op": op, "scope": scope, "path": abs, ...ctx() });
...
export function gateExecSync(cmd) {
	const b = String(cmd); const
		scope = "exec:" + b;

	if (redline(scope)) { return bernard(scope); }
	if (execOk(b)) { return; }
	const v = judicial({ "kind": "exec", "scope": scope, "bin": b, ...ctx() });
...
export function gateEvalSync(scope) {
	...
	const v = judicial({ "kind": "eval", "scope": scope, ...ctx() });
...
if (realFetch) {
	globalThis.fetch = async function(input, init) {
		...
		try { host = new URL(typeof input === "string" ? input : input.url).host; } catch {}
		await gateNet(host);
