...
import { existsSync, readFileSync, writeFileSync, unlinkSync, readSync } from "node:fs";
...
const scopeOf = (r) => { const k = KIND[r.permission] || r.permission; return r.value != null && r.value !== "" ? `${k}:${r.value}` : k; };
...
const persist = () => { try { writeFileSync(GRANTS, JSON.stringify([...granted], null, 0)); } catch {} };
...
function bernard(scope) {   // one-time human break-glass on the broker's OWN tty; fails closed headless
	...
	const buf = Buffer.alloc(64); let n = 0; try { n = readSync(0, buf, 0, 64, null); } catch {}
...
function decide(req) {
	...
	else { const v = judicial({ ...req, scope }); out = !v ? { result: "deny", reason: "no JUDICIAL decider (interactive ask unsupported over socket)" } : v.behavior === "allow" ? { result: "allow" } : { result: "deny", reason: v.message }; }
	if (out.result === "allow") { const isNew = !granted.has(scope); granted.add(scope); persist(); process.stderr.write(`[deno-broker] ${isNew ? "+ new " : ""}allow ${scope}\n`); }
