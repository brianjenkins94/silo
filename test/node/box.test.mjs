...
test("clean script runs under any policy", () => {
	const r = box("clean.mjs", { JUDICIAL: "deny" });
	assert.equal(r.status, 0, r.stderr);
	assert.match(r.stdout, /CLEAN 6/);
});

test("fs:write allowed by JUDICIAL=allow, denied by JUDICIAL=deny", () => {
...
test("fs:write allowed via ALLOW_FS allowlist (no JUDICIAL)", () => {
...
test("fs:write to credentials is BERNARD redline — fail-closed even with JUDICIAL=allow", () => {
...
test("exec: benign bin allowed, dangerous bin (rm) is redline fail-closed", () => {
...
test("net: external host denied by JUDICIAL=deny", () => {
	const r = box("net.mjs", { JUDICIAL: "deny", URL: "http://example.com/" });
	assert.notEqual(r.status, 0);
	assert.match(r.stderr, /DENIED net:example\.com/);
});

test("eval / Function / AsyncFunction / Generator are all redline fail-closed", () => {
	for (const MODE of ["eval", "fn", "asyncfn", "gen"]) {
		const r = box("codegen.mjs", { JUDICIAL: "allow", MODE });
