...
function silo(cwd, args, env = {}) {
	const r = spawnSync(TSX, [CLI, ...args], { cwd, encoding: "utf8", env: { ...process.env, NODE_OPTIONS: "", ...env } });
...
test("CI gate: no committed baseline → fail (with GitHub annotation)", () => {
...
test("CI gate: baseline present, no change → pass", () => {
	const dir = project(`import { readFileSync } from "node:fs";\nreadFileSync("x");\n`);
	try {
...
test("CI gate: capability expansion (new exec cap) → fail", () => {
	const dir = project(`import { readFileSync } from "node:fs";\nreadFileSync("x");\n`);
	try {
...
test("CI gate: a nested private workspace is ignored (its caps don't count as drift)", () => {
...
test("CI gate: a private project audited directly IS governed (target is never pruned)", () => {
...
test("audit annotates a capability introduced by likely-AI-authored code", () => {
	...
	try {
		...
		writeFileSync(path.join(dir, "cleanup.ts"),
			`// This function handles cleanup and runs a shell command.\n// Co-authored-by: Claude\nimport { execSync } from "node:child_process";\nexecSync("ls");\n`);
		const r = silo(dir, ["audit", "."]);
...
test("audit does NOT flag a capability introduced by terse human-authored code", () => {
...
test("CI gate: --ci never rubber-stamps (ignores --approve)", () => {
	const dir = project(`import { readFileSync } from "node:fs";\nreadFileSync("x");\n`);
	try {
		assert.equal(silo(dir, ["audit", "."]).status, 0);
