...
test("most functions carry a (terse) non-JSDoc doc comment → likely", () => {
	const r = provenance([
		"// get the token",
...
test("tagged JSDoc does not count → clean", () => {
	const r = provenance([
...
test("selective documentation (below threshold) → clean", () => {
...
test("comment-looking strings are not counted (oxc extraction)", () => {
	const r = provenance([
...
test("a comment mentioning a scoped package is NOT mistaken for JSDoc", () => {
...
test("a single documented function is enough → likely (no function-count floor)", () => {
...
test("a single UN-documented function → clean", () => {
	const r = provenance(`export function resolve(x) { return x; }\n`);
	assert.equal(r.verdict, "clean");
	assert.equal(r.documented, 0);
});

test("explicit attribution marker → likely", () => {
