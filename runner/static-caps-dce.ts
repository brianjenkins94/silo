 ...
 *
 * Run: ./node_modules/.bin/tsx scripts/runner/static-caps-dce.ts [targetFile]
 */
...
const target = path.resolve(process.argv[2] ?? path.join(ROOT, "runner/static-caps-dce.ts"));
...
async function capsOf(vite: any, tmp: string, exportName: string): Promise<string[]> {
	...
	const result = await vite.build({
		...
		build: {
			...
			rollupOptions: {
				input: entry,
				external: (id: string) => id.startsWith("node:") || /^[a-zA-Z@]/.test(id),
